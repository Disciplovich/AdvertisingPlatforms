...
Веб сервис, который позволяет хранить и возвращать списки рекламных площадок для заданной локации в запросе
...
Например:
```
https://localhost:8080/api/advertising/search?location=/ru/permobl.
```
Ответ будет возвращён в формате JSON с названиями, подходящих под запрос, площадок.
```
